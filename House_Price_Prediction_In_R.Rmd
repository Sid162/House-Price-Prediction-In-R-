...
train_missing_vals <- colSums(is.na(train_data))
train_missing_vals <- train_missing_vals[train_missing_vals > 0]
test_missing_vals <- colSums(is.na(test_data))
test_missing_vals <- test_missing_vals[test_missing_vals > 0]
...
train_plot <- plot_na_values(train_na_vals, "NA Values in Train File", "red")

# Plot NA values in the test file
test_plot <- plot_na_values(test_na_vals, "NA Values in Test File", "blue")
...
## There is zero zero rows means our data is imputed correctly there is no missing values.
...
## Since it's containing zero rows it means all the missing values have been replaced successfully.
